...
def decode_image_bytes_with_metadata(stego_img_bytes: bytes, password: str = None, progress_callback=None) -> tuple[bytes, dict]:
    """
    Decode and return (payload_bytes, metadata_dict).
    If password is required and incorrect, an exception will be raised.
    """
    img = Image.open(io.BytesIO(stego_img_bytes)).convert('RGB')
    pixels = list(img.getdata())
    bits = []
    total_pixels = len(pixels)

    for pix_idx, (r, g, b) in enumerate(pixels):
        bits.append(r & 1)
        bits.append(g & 1)
        bits.append(b & 1)
